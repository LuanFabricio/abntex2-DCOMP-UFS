...
estrutura cada neurônio reforça ou enfraquece a conexão com um dos neurônio da coluna anterior, como é demonstrado na
\autoref{cap_conceitos_ann_exemplo_ann}.

\begin{figure}[htb]
	\caption {\label{cap_conceitos_ann_exemplo_ann}Exemplo de uma ANN}
	\begin{center}
		\includegraphics[scale=0.25]{Imagens/exemplo-ann.pdf}
	\end{center}
	\legend {Fonte: Autor}
\end{figure}
...
Onde, Residual Network (ResNet) \cite{resnet} e AlexNet \cite{alexnet} são alguns exemplos de CNNs famosas.
...
(TODO: Revisar texto e adicionar figuras)

\section{Poda de redes neurais}
Redes neurais costumam usar muito recurso computacional e otimizar o uso dos recursos é muito importante para
aplicações de grande escala. A poda de redes neurais tem como objetivo principal remover pesos ou neurônios redundantes
ou irrelevantes para o problema, além de ajudar com a redução do \textit{overfitting}(superajuste).
(TODO: escrever sobre poda de redes neurais)

\section{Transferência de conhecimento}
Transferência de conhecimento, consiste em usar um modelo pré-treinado em uma base de dados específica e aproveitar
esse conhecimento adquirido durante esse treinamento para um novo conjunto de dados, é necessário que o problema do
\textit{dataset}(conjunto de dados) atual seja um subconjunto do \textit{dataset} que foi usado para treinar o modelo
base. Depois disso basta adaptar a camada de entrada e de "saída" (totalmente conectada) do modelo base, para que haja
um pré-processamento dos dados de entrada (antes deles serem passados para o modelo base) e é necessário definir a cama
da totalmente conectada.

 TODO: escrever sobre transferência de conhecimento
\section{Destilamento de conhecimento}
Destilamento de conhecimento ou \textit{knowledge distillation} \cite{hinton2015distilling}, é uma técnica de
treinamento que utiliza modelos, \emph{professor} que é um modelo mais robusto e pré-treinado para o problema, e o
modelo \emph{aluno} que é o modelo que (geralmente) é mais leve que o professor e não possui nenhum pré-treinamento.
...
\autoref{cap_conceitos}, que tem o nome \emph{\nameref{cap_conceitos}}. O número
do capítulo indicado é \ref{cap_conceitos}, que se inicia à
\autopageref{cap_conceitos}\footnote{O número da página de uma remissão pode ser
obtida também assim:
\pageref{cap_conceitos}.}.
